      ...
      <div class="atm-card">
        ...
        <template v-if="!confirmed && !success">
          <div class="card-title-row">
            ...
            <h3 class="atm-card-title">Deposit</h3>
          </div>
        ...
        <template v-else-if="confirmed && !success">
          <h3 class="atm-card-title">Confirm deposit</h3>
          <div class="confirm-box">
            <p class="confirm-label">Amount to deposit</p>
            <p class="confirm-amount">{{ formatAmount(parseFloat(amount)) }}</p>
          </div>
          <p class="atm-sub">to account ending …{{ iban?.slice(-8) }}</p>
          <Alert v-if="errorMessage" type="error" :message="errorMessage" />
          <div class="btn-row">
            <Button label="Confirm" variant="primary" size="md" :loading="isLoading" class="btn-flex" @click="handleDeposit" />
        ...
        <template v-else>
          <div class="success-state">
            ...
            <p class="atm-sub">Deposited {{ formatAmount(parseFloat(amount)) }} to your account.</p>
